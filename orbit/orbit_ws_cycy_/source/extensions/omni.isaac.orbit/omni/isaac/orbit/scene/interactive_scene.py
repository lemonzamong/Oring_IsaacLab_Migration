...
from omni.isaac.orbit.assets import Articulation, ArticulationCfg, AssetBaseCfg, RigidObject, RigidObjectCfg, SoftObject, SoftObjectCfg
...
class InteractiveScene:
    ...
    soft_objects: dict[str, SoftObject] = dict()
    """A dictionary of soft objects in the scene"""
    sensors: dict[str, SensorBase] = dict()
    """A dictionary of the sensors in the scene, such as cameras and contact reporters."""
    # geometry_objects: dict[str, GeometryPrimView] = dict()
    """A dictionary of geometry object in the scene, if you want to on/off collision of mesh"""
    extras: dict[str, XFormPrimView] = dict()
    
    # TODO ADD soft_objects: dict[str, SoftObject] = dict() ; CY
    # soft_objects: dicf[str, SoftPrimView] = dict()
    ...
    def reset(self, env_ids: Sequence[int] | None = None):
        ...
        for soft_object in self.soft_objects.values():
            soft_object.reset(env_ids)
        # -- sensors
    ...
    def keys(self) -> list[str]:
        ...
        """
        all_keys = ["terrain"]
        for asset_family in [self.articulations, self.rigid_objects, self.soft_objects, self.sensors, self.extras]:
    ...
    def __getitem__(self, key: str) -> Any:
        ...
        for asset_family in [self.articulations, self.rigid_objects, 
                             self.soft_objects, self.sensors, self.extras]:
                            #  self.soft_objects, self.sensors, self.geometry_objects, self.extras]:
    ...
    Internal methods.
    """
    def update_articulation_cfg(self, cfg):
        # HOWTOFIX?
        for asset_name, asset_cfg in cfg.items():
    ...
    def _add_entities_from_cfg(self):
        ...
        for asset_name, asset_cfg in self.cfg.__dict__.items():
            ...
            elif isinstance(asset_cfg, SoftObjectCfg):
                self.soft_objects[asset_name] = asset_cfg.class_type(asset_cfg)
            # elif isinstance(asset_cfg, GeometryObjectCfg):
            #     self.geometry_objects[asset_name] = GeometryPrimView(asset_cfg.prim_path, reset_xform_properties=False)
