...
class PseudoInverseJacobianAction(ActionTerm):
    r"""Velocity Controlled Task Space action term.

    This action term performs pre-processing of the raw actions using scaling transformation and then uses the
    Jacobian to convert the end-effector velocity commands to joint velocities.

    .. math::
        \text{action} = \text{scaling} \times \text{input action}
        \text{joint velocity} = J^{+} \times \text{action}

    where :math:`\text{scaling}` is the scaling applied to the input action, and :math:`\text{input action}`
    is the input action from the user, :math:`J` is the Jacobian over the articulation's actuated joints,
    and :math:`J^{+}` is the pseudoinverse of the Jacobian, and \text{joint velocity} is the desired joint velocity
    command for the articulation's joints.
    """

    cfg: actions_cfg.PseudoInverseJacobianActionCfg
    """The configuration of the action term."""
    _asset: Articulation
    """The articulation asset on which the action term is applied."""
    _scale: torch.Tensor
    ...
    def __init__(self, cfg: actions_cfg.PseudoInverseJacobianActionCfg, env: BaseEnv):
        # initialize the action term
        super().__init__(cfg, env)
    ...
    def apply_actions(self):
        # compute the current end-effector velocity if in relative mode
        if self.cfg.use_relative_mode:
            ee_vel_curr = self._compute_frame_velocity()
            # add the processed action to the current end-effector velocity
            ee_vel_target = ee_vel_curr + self._processed_actions
        else:
            # absolute mode
            ee_vel_target = self._processed_actions
        
        # compute the joint velocities from the Jacobian
        jacobian = self._compute_frame_jacobian()
        joint_vel = torch.bmm(jacobian.pinverse(), ee_vel_target.unsqueeze(-1)).squeeze(-1)
        # set the joint velocity command
        self._asset.set_joint_velocity_target(joint_vel, self._joint_ids)
    ...
    def _compute_frame_velocity(self):
        """Computes the current velocity of the end-effector in the root frame.

        Returns:
            The end-effector's current velocity in the root frame.
        """
        # obtain quantities from simulation
        ee_lin_vel_w = self._asset.data.body_state_w[:, self._body_idx, 7:10]
        ee_ang_vel_w = self._asset.data.body_state_w[:, self._body_idx, 10:13]
        ee_vel_w = torch.cat((ee_lin_vel_w, ee_ang_vel_w), dim=-1)
