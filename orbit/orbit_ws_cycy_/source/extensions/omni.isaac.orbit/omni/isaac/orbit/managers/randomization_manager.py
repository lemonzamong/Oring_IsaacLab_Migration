...
from omni.isaac.orbit_assets.franka_gripper import PANDA_CFG
import carb
...
class RandomizationManager(ManagerBase):
    ...
    def randomize(self, mode: str, env_ids: Sequence[int] | None = None, 
                  dt: float | None = None, scene: None = None):
        ...
        for index, term_cfg in enumerate(self._mode_term_cfgs[mode]):
            ...
            # elif mode == "pre_reset":
            #     if scene is None: 
            #         raise ValueError("sibal.")
            #     term_cfg.func(self._env, env_ids, **term_cfg.params)
            #     aa = ArticulationCfg(PANDA_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot"))
            #     cfg = {"robot":aa}
            #     scene.update_articulation_cfg(cfg)
            # else:
            term_cfg.func(self._env, env_ids, **term_cfg.params)
