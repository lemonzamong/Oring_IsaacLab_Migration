            ...
            for name, annotators in self._rep_registry.items():
                if name == 'pointcloud':
                    for index in env_ids:
                        ...
                        # If you want segementation different object in each env, change this code
                        semantic_label = {self.cfg.semantic_types[0]: self.cfg.semantic_labels[0]}
                        ...
                        if semantic_label in sementic_idToLabels.values():
                            self.semantic_idx = int(self._find_value(sementic_idToLabels, semantic_label))
                        indices = torch.tensor([idx for idx, value in enumerate(info["pointSemantic"]) if value == self.semantic_idx])

                        min_points = 256

                        num_points = len(indices)
                        idx = torch.randperm(num_points)
                        if num_points > min_points and data.shape[0] > 0:
                            if num_points < self.cfg.pointcloud_size:
                                duplicate_idx = torch.randint(num_points, (self.cfg.pointcloud_size - num_points,))
                                idx = torch.cat((idx, duplicate_idx))
                            seg_data = data[idx[:self.cfg.pointcloud_size]]
                        else:
                            seg_data = torch.zeros((self.cfg.pointcloud_size, 3))
        ...
        for name, annotators in self._rep_registry.items():
            ...
            if name == "pointcloud":
                ...
                for index in self._ALL_INDICES:
                    ...
                    # If you want segementation different object in each env, change this code
                    segment_name = {self.cfg.semantic_types[0]: self.cfg.semantic_labels[0]}
                    
                    # get id of segmentation label
                    if segment_name in sementic_idToLabels.values():
                        self.semantic_idx = int(self._find_value(sementic_idToLabels, segment_name))
                    else: 
                        data_all_cameras.append(torch.zeros((self.cfg.pointcloud_size, 3)))
                        print("semantic labed not found")
                        continue
                        # raise RuntimeError(
                ...
                # # sample the data to have the same length
                    
                pointcloud_size = self.cfg.pointcloud_size
                
                for i in self._ALL_INDICES:
                    num_points = len(data_all_cameras[i])
                    idx = torch.randperm(num_points)
                    if num_points < pointcloud_size:
                        duplicate_idx = torch.randint(num_points, (pointcloud_size - num_points,))
                        idx = torch.cat((idx, duplicate_idx))
                    data_all_cameras[i] = data_all_cameras[i][idx[:pointcloud_size]]
            

                # max_obs = max(max(len(data) for data in data_all_cameras),4096)
                # for i in self._ALL_INDICES:
                #     data_length = len(data_all_cameras[i])
                #     if data_length < max_obs:
                #         tmp = torch.zeros([max_obs, 3], device=self.device) 
                #         tmp[:data_length] = data_all_cameras[i]
                #         data_all_cameras[i] = tmp
                #     elif data_length > max_obs:
                #         data_all_cameras[i] = data_all_cameras[i][:max_obs]
