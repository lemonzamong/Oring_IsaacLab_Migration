...
def root_pose_in_world_frame(env: BaseEnv, robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
End effector state.
"""
def ee_pose_in_robot_frame(env: BaseEnv, robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    robot: RigidObject = env.scene[robot_cfg.name]
    ee_pos_w, ee_quat_w = ee_pose_in_world_frame(env, robot_cfg)
    root_pose_w = root_pose_in_world_frame(env, robot_cfg)
...
def ee_pose_in_world_frame(env: BaseEnv, robot_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    robot: RigidObject = env.scene[robot_cfg.name]
    ee_pose_w = robot.data.body_state_w[:, -1, 0:7]
